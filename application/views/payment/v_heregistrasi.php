<div class="container-fluid">
	...
	<div class="row heading-bg">
		<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
			<h5 class="txt-dark">Ringkasan Heregistrasi <i class="fa fa-money mr-20"></i></h5>
		</div>
							...
							<table class="table table-hover mb-0">
								<thead>
									<tr>
										...
										<th class="txt-dark">HEREGISTRASI</th>
										<th class="txt-dark">PEMBAYARAN</th>
										<th class="txt-dark">SISA PIUTANG</th>
...
<script type="text/javascript">
	...
	function show_heregistrasi(latest_periode){
		...
		$.ajax({
			type : "POST",
			url     : '<?php echo base_url();?>index.php/Payment/show_sum_heregistrasi',
			dataType : "JSON",
			data : {id_periode:latest_periode},
			success: function(data){
				for(i=0; i<data.length; i++){
					total_lunas = data[0].TOTAL;
					total_piutang = data[0].KEKURANGAN;
					total_bayar = data[0].JUMLAH_HARUS_DIBAYAR;
				}
			}
		});
		$.ajax({
			type : "POST",
			url     : '<?php echo base_url();?>index.php/Payment/show_heregistrasi',
			dataType : "JSON",
			data : {id_periode:latest_periode},
			success: function(data){
				var i;
				...
				for(i=0; i<data.length; i++){
					...
					html +=
					'<td>'+data[i].NIS+'</td>'+
					'<td>Rp. '+data[i].HEREGISTRASI+'</td>'+
					'<td>Rp. '+data[i].DIBAYAR+'</td>';
					if (data[i].KEKURANGAN == 0) {
						html += '<td>LUNAS</td>';
					} else {
						html += '<td>Rp. '+data[i].KEKURANGAN+'</td>';
					}
					html += 
					'<td>'+
